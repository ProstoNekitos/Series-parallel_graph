...
public:
    ...
    SPGraph(Node<T>* source, Node<T>* terminal, size_t size);
    ~SPGraph();
...
SPGraph<T>::SPGraph(const T &src, const T &trm)
    : source(new Node<T>(src)), terminal(new Node<T>(trm)), capacity(2)
{
...
SPGraph<T>::SPGraph(SPGraph &&other) noexcept
    : source(std::move(other.source)), terminal(std::move(other.terminal)), capacity(std::move(capacity)){}

template<class T>
SPGraph<T>::SPGraph(Node<T>* src, Node<T>* trm, size_t size)
{
    this->source = src;
    this->terminal = trm;
    capacity = size;
